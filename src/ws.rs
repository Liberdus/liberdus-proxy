...
fn generate_uuid() -> String {
    ...
    format!(
        ...
        u32::from_be_bytes([random_bytes[0], random_bytes[1], random_bytes[2], random_bytes[3]]),
        ...
        u64::from_be_bytes([
            ...
            random_bytes[14], random_bytes[15], 0, 0
...
pub async fn listen(
    ...
    let listener = match tokio::net::TcpListener::bind(format!("0.0.0.0:{}", config.http_port.clone()+1)).await {
        Ok(l) => l,
        Err(e) => {
    ...
    println!("Websocket Listening on: {}", listener.local_addr().expect("Couldn't bind to a port"));
...
async fn handle_stream<StreamLike>(
    ...
    where StreamLike: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static
{
    ...
    let ws_stream = match tokio_tungstenite::accept_async(stream).await {
        ...
        Err(e) => {
            ...
            return Err(std::io::Error::new(std::io::ErrorKind::Other, "Error accepting websocket"));
        }
    };
                ...
                Ok(msg) => {
                    match msg {
                        Message::Close(_) => {
                            ...
                            subscription_manager_long_lived.unsubscribe_all(&socket_id).await;
                            drop(guard);
                            break;
                        },
                        Message::Text(msg) => {
                            ...
                            match parsed.method {
                                Methods::ChatEvent => {
                                    let e = handle_subscriptions(parsed, tx.clone(), Arc::clone(&subscription_manager_long_lived), id.clone()).await;
                                    if let Err(e) = e {
...
async fn handle_subscriptions(
    ...
    match msg.params.0 {
        subscription::SubscriptionActions::Subscribe => {
            let status = subscription_manager.subscribe(&socket_id, msg.params.1.as_str()).await;
        ...
        subscription::SubscriptionActions::Unsubscribe => {
            let status = subscription_manager.unsubscribe(&socket_id, msg.params.1.as_str()).await;
